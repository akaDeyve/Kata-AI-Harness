...
function parseAndRenderContent(content, showToast) {
  ...
  return parts.map((part) => {
    ...
    return <CodeBlock key={part.key} language={part.language} code={part.code} showToast={showToast} />
  })
}
...
function partLanguageLabel(lang) {
  const map = { js: 'JavaScript', ts: 'TypeScript', jsx: 'JSX', tsx: 'TSX', html: 'HTML', css: 'CSS', json: 'JSON', sh: 'Shell' }
...
function ScoreBar({ score }) {
  ...
  return (
    <div className="flex items-center gap-3 mb-4">
      <span className={`text-xl font-bold min-w-[6rem] ${
...
function AnimatedSection({ children }) {
  ...
  return (
    <div className={`transition-all duration-400 ease-out ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
      {children}
    </div>
  )
}
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full text-center gap-3">
      ...
      <p className="text-sm text-[#858585] max-w-md leading-relaxed">
        Schreibe deinen Code und klicke auf <strong className="text-[#569cd6]">&quot;AI-Feedback&quot;</strong> fuer eine Bewertung
        oder auf <strong className="text-[#2d6a4f]">&quot;Korrektur&quot;</strong> fuer eine verbesserte Version.
      </p>
    </div>
  )
}
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {isCorrectionLoading ? (
              ...
              <span className="inline-flex items-center gap-1.5"><Wrench size={14} /> Korrektur</span>
            )}
          </button>
          <button
          ...
          >
            {isLoading ? (
              ...
              <span className="inline-flex items-center gap-1.5"><Robot size={14} /> AI-Feedback</span>
            )}
          </button>
        </div>
      </div>
      ...
      <div className="h-48 overflow-y-auto p-4">
        ...
        {isLoading && (
          <div className="flex items-center justify-center h-full">
            <div className="text-center text-[#858585]">
              ...
              <p className="text-sm mt-3 animate-pulse">Analysiere deinen Code...</p>
            </div>
          </div>
        )}
        ...
        {isCorrectionLoading && (
          <div className="flex items-center justify-center h-full">
            <div className="text-center text-[#858585]">
              <Spinner size="w-8 h-8" />
              <p className="text-sm mt-3 animate-pulse">Erstelle Korrekturvorschlag...</p>
            </div>
          </div>
        )}
          ...
          <AnimatedSection>
            ...
            {feedback.isError ? (
              <div className="flex items-start gap-3 p-3 rounded-lg bg-red-900/20 border border-red-800/30">
                <span className="text-red-400 shrink-0"><Warning size={18} /></span>
                <div>
                  <p className="text-sm font-medium text-red-300 mb-1">Fehler</p>
                  <p className="text-sm text-red-200/80 whitespace-pre-wrap">{feedback.content}</p>
                </div>
              </div>
            ) : (
          ...
          <AnimatedSection>
            {correction.isError ? (
              <div className="flex items-start gap-3 p-3 rounded-lg bg-red-900/20 border border-red-800/30">
                ...
                <div>
                  <p className="text-sm font-medium text-red-300 mb-1">Fehler</p>
                  <p className="text-sm text-red-200/80 whitespace-pre-wrap">{correction.content}</p>
                </div>
              </div>
            ) : (
              <div>
                {parseAndRenderContent(correction.content, showToast)}
              </div>
            )}
          </AnimatedSection>
        )}
