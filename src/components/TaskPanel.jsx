...
export default function TaskPanel({ task }) {
  if (!task) {
    return (
      <div className="bg-s1 border-b border-borderc p-6 flex items-center justify-center text-t2 font-sans shrink-0"
        style={{ minHeight: '120px' }}>
        <div className="text-center">
          <Puzzle size={36} className="text-t2 mb-1" />
          <p className="text-base mb-1 text-text">Wähle eine Aufgabe aus der Seitenleiste</p>
          <p className="text-xs text-t3">Praktische Code-Muster für den Job-Alltag</p>
        </div>
      </div>
    )
  }
      ...
      <AnimatedContent taskId={task.id}>
        <div className="flex items-center gap-3 mb-3">
          ...
          <span className={`text-xs px-2 py-0.5 rounded font-medium ${
            task.difficulty === 'Leicht' ? 'bg-green/30 text-green' :
            task.difficulty === 'Mittel' ? 'bg-yellow/30 text-yellow' :
            'bg-red/30 text-red'
          }`}>
            {task.difficulty}
          </span>
