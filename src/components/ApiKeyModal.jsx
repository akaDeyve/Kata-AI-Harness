...
import { GeminiLogo, OpenRouterLogo, OllamaLogo, OpenCodeLogo, Check, Close } from './Icons'

function getProviderIcon(id, className) {
  switch (id) {
    case 'gemini': return <GeminiLogo size={26} className={className} />
    case 'openrouter': return <OpenRouterLogo size={26} className={className} />
    case 'ollama': return <OllamaLogo size={26} className={className} />
    case 'opencode': return <OpenCodeLogo size={26} className={className} />
    default: return null
  }
}
...
export default function ApiKeyModal({ config, onSave, onClose }) {
  ...
  const defaultProvider = displayProviders.find(p => p.id === config.apiType)?.id || displayProviders[0]?.id || DEFAULT_PROVIDER
  const [selectedId, setSelectedId] = useState(defaultProvider)
  const [key, setKey] = useState(config.key)
  const [customUrl, setCustomUrl] = useState(
    displayProviders.some(p => p.defaultUrl === config.baseUrl) ? '' : config.baseUrl
  )
  const [customModel, setCustomModel] = useState(
    displayProviders.some(p => p.defaultModel === config.model) ? '' : config.model
  )
  ...
  if (displayProviders.length === 0) {
    return (
      <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 font-sans" onClick={onClose}>
        <div className="bg-s1 border border-borderc rounded-xl shadow-2xl w-full max-w-sm mx-4 p-6 text-center" onClick={e => e.stopPropagation()}>
          <div className="text-3xl mb-3">⚠️</div>
          <h3 className="text-base font-semibold text-text mb-2">Keine KI-Anbieter aktiv</h3>
          <p className="text-sm text-t2 mb-4">Gehe zu den Einstellungen und aktiviere mindestens einen KI-Anbieter.</p>
          <button onClick={onClose} className="px-4 py-2 text-sm bg-accent text-white rounded-lg hover:bg-accent/90">Schließen</button>
        </div>
      </div>
    )
  }
        ...
        <div className="p-5">
          ...
          {activeProvider?.id === 'ollama' && (
            <div className="mb-4 p-3 rounded-lg bg-s3 border border-borderc text-xs text-t2">
              ...
              <p className="mt-1.5 text-t3">Kein API-Key erforderlich – läuft lokal auf deinem Rechner.</p>
            </div>
          )}
